 ...
 */
public class mergeVersusQuickTiming {
...
//		try (FileWriter fw = new FileWriter(new File("thresholdTiming.tsv"))) {
        ...
        for (int exp = 2; exp <= SAMPLE_SIZE; exp += 1000) {
            //create an ArrayList of integers to time
            ArrayList<Integer> msArray = SortUtil.generateAverageCase(exp);
            ...
            for (int iter = 0; iter < exp; iter++) {
                ...
                //method to test
                SortUtil.quicksort(msArray, (lhs, rhs) -> lhs.compareTo(rhs));
