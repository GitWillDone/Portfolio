...
struct AABB {
    ...
    //default construct to be infinite, which is useful for KDTree
    AABB() {
        for (int i = 0; i < Dimension; ++i) {
    ...
    //returns true if the point is inside the input bounding box.
    bool containsPoint(const Point<Dimension> &p, const AABB<Dimension> &box) {
        //check maxP of bounding box first
        if (p.point[0] > box.maxs[0]) { //point.x is greater than maxP x
            return false;
        } else if (p.point[1] > box.maxs[1]) { //point.y is great than maxP y
            return false;
        }

        //check minP of bounding box
        if (p.point[0] < box.mins[0]) { //point.x is less than minP x
            return false;
        } else if (p.point[1] < box.mins[1]) { //point.y is less than minP y
            return false;
        }

        return true; //passes all above tests to see if the point is inside the input bounding box
    }

    //returns true if the two AABBs instersect
    bool intersectsAABB(const AABB<Dimension> &lhs, const AABB<Dimension> &rhs) {
        //maxes and mins of rhs box
        Point<Dimension> rMax = getBounds(rhs).maxs;
        Point<Dimension> rMin = getBounds(rhs).mins;

        return (containsPoint(rMax, lhs) || containsPoint(rMin, lhs));
    }
};
...
template<int Dimension>
Point<Dimension> operator-(const Point<Dimension> &lhs, const Point<Dimension> &rhs) {
    std::array<float, Dimension> retPoint;

    for (int i = 0; i < Dimension; ++i) {
        retPoint[i] = (lhs[i] - rhs[i]);
    }
    return Point<Dimension>{retPoint};
}

template<int Dimension>
Point<Dimension> operator-(const Point<Dimension> &lhs, const float &rhs) {
    std::array<float, Dimension> retPoint;

    for (int i = 0; i < Dimension; ++i) {
        retPoint[i] = (lhs[i] - rhs);
    }
    return Point<Dimension>{retPoint};
}
...
template<int Dimension>
Point<Dimension> operator/(const Point<Dimension> &lhs, int rhs) {
    std::array<float, Dimension> retPoint;
...
template<int Dimension>
std::array<int, Dimension> pointToIntArr(const Point<Dimension> &inputPoint) {
    std::array<int, Dimension> retVec;
    for (int i = 0; i < Dimension; ++i) {
        retVec[i] = inputPoint.point[i];
    }
    return retVec;
}