...
public:
    ...
    KDTree(const std::vector<Point<Dimension> > &points) {
        std::vector<Point<Dimension>> myPoints = points;
        root = std::unique_ptr<Node < 0>>
        (new Node<0>(myPoints.begin(), myPoints.end()));
    }
...
private:
    ...
    void rangeQueryHelper(Node<SplitDimension> node, const Point<Dimension> &p, float radius,
                          std::vector<Point<Dimension> > query) const {
    ...
    template<int SplitDimension>
    void
    KNNHelper(const std::unique_ptr<Node<SplitDimension>> &node, const Point<Dimension> &p, int k, AABB<Dimension> aabb,
              std::vector<Point<Dimension> > &heap) const {
        if (heap.size() < k) {
...
public:
    ...
    std::vector<Point<Dimension> > rangeQuery(const Point<Dimension> &p, float radius) const {
        std::vector<Point<Dimension> > query = std::vector<Point<Dimension>>();
        rangeQueryHelper(root, p, radius, query);
        return query;
    }

    std::vector<Point<Dimension>> KNN(const Point<Dimension> &p, int k) const {
        std::vector<Point<Dimension>> heap = std::vector<Point<Dimension>>();
